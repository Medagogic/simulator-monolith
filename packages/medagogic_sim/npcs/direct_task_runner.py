...
class DirectTaskRunner:
    def __init__(self, context: ContextForBrains, task: TaskCall):
    ...
    def getStartingString(self) -> str:
        ...
        return f"Started `{self.task.call_data.name} ({params_str})`"
    

    def getConsequenceString(self) -> str:
        params_str = ", ".join(self.task.call_data.params)
        return f"Finished `{self.task.call_data.name} ({params_str})`"


    def getUpdateString(self) -> str:
        params_str = ", ".join(self.task.call_data.params)
        return f"Performed action: `{self.task.call_data.name} ({params_str})`"
    ...
    async def run(self) -> None:
        ...
        if action_type in [ActionType.INTERVENTION, ActionType.PREPARATION]:
            ...
            self.add_pending_task(self.connect_devices())
        else:
...
""".strip()
        ...
        self.context.history.add_event(Evt_TaskConsequence(npc_name="Sam", content=self.getConsequenceString()))
    ...
    async def __generate_basic_completion_dialog(self) -> None:
        ...
        prompt = f"""
The user will inform you of an action which you have just completed, along with some information about the new state of the patient and simulation.
Provide an update to the user about your completed action. Keep your response very brief. Be passive rather than active, your response should not guide the user. Compare Initial Patient State and Simulation Results below to observe any consequences of the action..
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        task_runner = DirectTaskRunner(context, context.action_db.get_action_from_call("Connect pulse oximeter", "Let's get the pulse oximeter connected"))
